...
## Nuovo in v0.12.0 — Versione di ripristino della copertura
...
### Cosa research-os non è (e cosa la versione v0.12.0 non pretende di essere)

- Non è stato dimostrato che il sistema funzioni in modo autonomo su nuove installazioni. La versione v0.12.0 risolve i problemi riscontrati nella fase di test "aloneness" (v0.3) (autonomia dimostrata a livello di sicurezza, ma non ancora a livello di copertura); la versione v0.4 eseguirà ulteriori test in una sessione separata e potrebbe rivelare ulteriori correzioni. La versione v0.12.0 è un prerequisito per la versione v0.4, ma non è una prova di funzionalità autonoma.
- Non è stata testata in condizioni reali da utenti esterni, al di là delle prove interne e dei tre cicli di test "aloneness". Sono state completate sei prove interne, di cui una auto-referenziale e cinque relative a domini esterni (ComfyUI, XRPL, Godot, calibrazione dei revisori, revisore deterministico), oltre ai cicli di test "aloneness" delle versioni v0.1, v0.2 e v0.3, che hanno evidenziato 17 problemi (R-001 fino a R-005 risolti nella v0.10.0, R-007 fino a R-011 risolti nella v0.11.0, R-012 fino a R-017 risolti nella v0.12.0). L'utilizzo del sistema da parte di operatori esterni su larga scala è un obiettivo futuro.
- Non è uno strumento completo per la creazione di pacchetti. La versione v0.12.0 eredita le funzionalità di gestione delle sezioni (`synth section`) e dei pacchetti parziali (`synth pack --partial`) introdotte nella versione v0.9, con una chiara indicazione della prontezza del pacchetto. La creazione di pacchetti completi richiede ancora un pacchetto con l'attributo `synthesis_ready` e la creazione manuale (o tramite Cowork) basata sugli ID delle richieste accettate, utilizzando `synth workspace`.
- Non è una garanzia di affidabilità di alcun modello di revisione. La versione v0.12.0 non include, di default, un profilo di revisore "trusted_baseline"; i ricevuti di calibrazione sono una prova, non una garanzia. I ricevuti di calibrazione esistenti, risalenti alla versione v0.6.0, sono precedenti all'architettura MCP (v0.8.0) e non sono stati aggiornati in base al percorso MCP. Consultare la [pagina del manuale sulla calibrazione dei revisori](https://mcp-tool-shop-org.github.io/research-os/handbook/reviewer-calibration/).
- Non è privo di elementi storici nei pacchetti "congelati". I pacchetti "congelati" precedenti alla versione v0.4 contengono `research_os_version: '0.1.0'` a causa di una costante predefinita introdotta prima della versione v0.4; la correzione è stata implementata nella versione v0.4.0, ma i pacchetti "congelati" precedenti non possono essere modificati in base alla Legge 15 (vedere [`handbook/known-limitations`](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/)).
- Non è dotato di attestazione di provenienza su npm. L'attestazione di provenienza tramite Sigstore è prevista per una versione futura; verificare i pacchetti npm della versione v0.12.0 tramite package-shasum e l'hash del commit della release su GitHub.
- Non rappresenta un miglioramento rispetto alle soluzioni cloud. La valutazione comparativa tra la versione locale e quella cloud, disponibile in `local-first-vs-cloud-research/` (versione v0.7.x), ha evidenziato i vantaggi delle soluzioni cloud in termini di leggibilità e carico di lavoro per gli operatori; la versione v0.12.0 non afferma di aver superato tali limitazioni.

### Limitazioni note

La versione v0.12.0 include tre limitazioni note, visibili agli operatori, ereditate dalle versioni precedenti. Ognuna di esse è documentata nella [pagina del manuale sulle limitazioni note](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/) e nel file [CHANGELOG.md](CHANGELOG.md). Nessuna di queste limitazioni impedisce il rilascio; tutte hanno una procedura di risoluzione o mitigazione definita.
...
- **B-E-004 — L'attestazione "npm provenance" è stata rimandata a una versione futura.** Il file "tarball" npm della versione v0.12.0 verifica solo tramite "package-shasum". La migrazione del processo di pubblicazione a un flusso di lavoro CI con sigstore OIDC è in conflitto con la disciplina di "traduzione prima della pubblicazione" (TranslateGemma 12B viene eseguito localmente); la migrazione è prevista per una versione futura. Verificare i pacchetti npm della versione v0.12.0 tramite "package-shasum" e l'hash del commit della release su GitHub.
- **B-A-003 — La migrazione dello schema della versione dell'indicizzatore è documentata, ma non imposta.** La versione v0.12.0 include un intero `SCHEMA_VERSION` per la scrittura, ma non include un componente per la migrazione in lettura. In caso di aggiornamento documentato di `SCHEMA_VERSION`, eliminare il file `.research-os/index.sqlite` e rieseguire il comando `research-os index build --all`. Il pacchetto stesso non è interessato; l'indicizzatore è uno strato di accelerazione basato su prove e dichiarazioni (Legge 8); la ricostruzione è idempotente.

**Nella versione v0.12.0, non è ammesso alcun profilo di revisore "trusted_baseline".** Questa è una scelta deliberata in termini di gestione della fiducia, e non una lacuna: le ricevute di calibrazione presenti nel repository (`hermes-two-pass=failed`, `mistral-nemo-two-pass=conditional_pass`, `hermes-single-pass=comparison_only`, `hermes-two-pass-deterministic=failed`) registrano le prove. La fiducia si guadagna attraverso ripetuti tentativi di recupero in caso di errori simulati, e non viene data per scontata. Queste ricevute risalgono a un'architettura MCP precedente alla versione v0.8.0 e non sono state riallineate secondo il percorso MCP.
