...
**Richiede [`ollama-intern-mcp@^2.4.0`](https://github.com/mcp-tool-shop-org/ollama-intern-mcp) in esecuzione localmente** per l'estrazione, la classificazione, la revisione e la scoperta di modelli linguistici di grandi dimensioni (LLM). Il server MCP viene rilevato tramite la variabile d'ambiente `OLLAMA_INTERN_MCP_BIN` o tramite il percorso di sistema (PATH). Il modello predefinito è `hermes3:8b`; è possibile sovrascriverlo con `OLLAMA_INTERN_MODEL=<modello>` (o tramite l'opzione `--model <nome>` per ogni chiamata). Impostare `OLLAMA_HOST` se Ollama non è in esecuzione sull'indirizzo predefinito `localhost:11434`.
...
La versione 0.8.0 è una release incentrata sul ripristino dell'architettura: research-os ora utilizza `ollama-intern-mcp@^2.4.0` come sottosistema locale per l'elaborazione delle prove, utilizzato per l'estrazione di affermazioni, aggiunge l'applicazione della pertinenza contestuale, conserva le prove non pertinenti come escluse anziché utilizzabili e aggiunge la sintesi delle prove a livello di sezione per le sezioni idonee per il processo di validazione nei pacchetti che richiedono correzioni.

Consultare i file [`docs/release-notes/v0.8.0.md`](docs/release-notes/v0.8.0.md) e [CHANGELOG.md](CHANGELOG.md).

## Stato

**v0.8.0 — Ripristino dell'architettura + Pertinenza contestuale** — pubblicata su npm come `@mcptoolshop/research-os@0.8.0`, 12 maggio 2026. La versione 0.8.0 è una release incentrata sul ripristino dell'architettura: research-os ora utilizza `ollama-intern-mcp@^2.4.0` come sottosistema locale per l'elaborazione delle prove utilizzato per l'estrazione di affermazioni (precedentemente il file README dichiarava questa dipendenza, ma il codice aveva delle implementazioni interne dirette a Ollama che la bypassavano fin dalla versione 0.1 — la versione 0.8.0 risolve questa discrepanza). Aggiunte: sottosistema client MCP (`OLLAMA_INTERN_MCP_BIN` variabile d'ambiente + rilevamento PATH + trasporto tramite StdioClient); valutazione delle prove a livello di affermazione tramite `ollama_extract` con schema a 4 etichette (`supports_section` / `off_topic` / `background_only` / `source_chrome`); nuova decisione di revisione `frame_excluded` (la revisione salta l'LLM per le affermazioni escluse, generando un `ClaimReview` sintetico); lo schema `ClaimSchema` include `frame_excluded` + `frame_exclusion_reason` (enum a 4 valori, inclusi `critic_unavailable` per i fallimenti dello stato del sistema) + `frame_exclusion_rationale`; sintesi delle prove a livello di sezione tramite `synth section <id>` per le sezioni idonee per il processo di validazione nei pacchetti che richiedono correzioni (indice delle citazioni delle prove — ID dell'affermazione → asserzione → estratto delle prove → URL della fonte — NON testo narrativo); il processo di validazione rispetta il registro delle sovrascritture delle schede della fonte tramite `getEffectivePublisher` / `getEffectiveSourceType` (obiettivo raggiunto con la versione 0.7.1); il valore predefinito di `DEFAULT_WINDOW_CHARS` è stato modificato da 5000 a 3000 (dimensione ottimizzata per `hermes3:8b` con un contesto di lavoro di 8K nel profilo `dev-rtx5080`); la politica di fallimento "soft" per le chiamate al valutatore è stata invertita (qualsiasi dei 5 modi di fallimento — trasporto / analisi / etichetta non valida / motivazione vuota / timeout — porta di default a `frame_excluded: true` con la motivazione `critic_unavailable`, anziché all'ammissione); la semantica della promozione: le affermazioni `frame_excluded` non bloccano la promozione della sezione; la gestione della collaborazione espone `frame_excluded` come un bucket separato dagli elementi accettati / da correggere / rifiutati. **Richiede `ollama-intern-mcp@^2.4.0`**. 1013 test superati su 1013 (da 901 a 1013, +112 test). **Tutti e quattro i pacchetti "congelati" verificano l'identità byte per byte rispetto alle baseline della versione 0.3.3.** **Non è una release della versione 1** — il lavoro per la preparazione alla versione 1 continua; consultare il file [`docs/roadmap.md`](docs/roadmap.md). Consultare i file [`docs/release-notes/v0.8.0.md`](docs/release-notes/v0.8.0.md) e [CHANGELOG.md](CHANGELOG.md).
