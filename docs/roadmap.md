...
**Status: IN PROGRESS.** Pack #2 of 3 closeout COMPLETE (2026-05-10). Pack admitted to `mcp-tool-shop-org/research-packs` as the third package via `research-os pack publish` against the npm-published v0.3.2 binary; all three packages PASS `verify-pack.mjs` with prior fingerprints stable. Proof doc at [`docs/experiment-3-pack-2-proof.md`](experiment-3-pack-2-proof.md). Three v0.3.x releases earned by the arc: **v0.3.0** (`--detector` flag), **v0.3.1** (section-scoped source waivers + reviewer acknowledgement), **v0.3.2** (normalized accepted-claim accounting via `getEffectiveAcceptedClaimIds`). One more external-domain pack (pack #3 of 3) required for closure — topic TBD (advisor decision). Candidate framings to contrast with ComfyUI (community-distribution-shaped) and XRPL (canonical-protocol-shaped): protocol-shaped non-blockchain (DNS / BGP / IPv6 / OAuth 2.1 / WebTransport) tests waiver discipline beyond blockchain protocol-defining bodies; documentation-heavy non-ML (embedded toolchain / scientific reproducibility / IDE plugins) tests community-tier accessibility limits beyond ML; mixed-shape (clinical-evidence durability / hardware supply chain) is highest-risk with the strongest generalization signal. **Experiment 3 NOT marked closed.** Closure requires three external-domain packs end-to-end without breaking changes; pack #3 is still ahead.
