...
## Nueva versión v0.10.0: Lanzamiento de corrección de errores relacionados con el trabajo individual del operador

La versión v0.10.0 soluciona las condiciones de fallo de la puerta de "trabajo individual del operador" de la versión v0.1, detectadas el 15 de mayo de 2026 (`operator_aloneness_dst_v0.1`, FALLO). Cuatro mejoras se implementan simultáneamente: alineación de la ruta de recuperación, herramienta de línea de comandos (CLI) para la corrección del alcance, endurecimiento de la auditoría de la tarjeta de origen y estado de recopilación preciso. La puerta de la versión v0.1 FALLÓ porque un operador externo se encontró con tres problemas independientes: el sistema de recuperación recomendó una ruta de desbloqueo incorrecta, no existía una herramienta de línea de comandos para corregir reclamaciones con `scope=null`, y la auditoría de la tarjeta de origen pasó una prueba de bot fabricada, extraída de un fragmento de 1035 bytes del bot APA Incapsula. La versión v0.10.0 soluciona estos problemas.
...
Antes de la extracción de reclamaciones, la auditoría de la tarjeta de origen ahora aísla fragmentos de pruebas de bots/CAPTCHA (`bot_check_or_captcha_detected`, FALLO GRAVE) y fuentes de baja calidad/alta extracción (`extraction_suspect_word_count_mismatch`, ADVERTENCIA Y AISLAMIENTO). Los operadores pueden anular esta acción a través del nuevo campo `clear_severities[]` en el registro existente de anulación de la tarjeta de origen de la versión v0.4.

El progreso de la recopilación ahora muestra un valor de la enumeración `gather_outcome` (con los valores: `ok | fetch_failed | extraction_skipped | extraction_failed | bot_check_detected`) en cada recibo de recopilación. La frase confusa de la versión v0.1, `"Failed (ok HTTP 200)"`, ya no aparece; los archivos PDF ahora se muestran como `extraction_skipped` (omitida), en lugar de `Failed` (fallida).

### Límite legal

El proceso de corrección es complementario. Las restricciones establecidas se mantienen: el valor mínimo aceptable de `accepted_claim_floor` sigue siendo inalterable; el sistema de asesoramiento de recuperación aún se niega a recomendar `apply_waiver` para fallos ineludibles. La enumeración `FailureShape` cerrada no se ha modificado; R-002 solo agrega la ruta del estado de la puerta a las nueve formas existentes. `RECOVERY_ACTIONS` aumenta de 7 a 8 valores cerrados (se agrega `repair_claim_scope`). El aislamiento por gravedad nunca promueve automáticamente más allá de la puerta de auditoría sin una anulación explícita del operador (el nuevo campo `clear_severities[]` es una decisión registrada del operador y solo permite adiciones).

La regresión del paquete "congelado" es idéntica en bytes a las versiones base v0.3.3 para los cuatro paquetes "congelados" (séptimo lanzamiento consecutivo en el que esto se cumple).

### Lo que la versión v0.10.0 NO promete

- Preparación para la versión 1.
- Verificación de la puerta de "trabajo individual del operador" de la versión 0.2. La versión 0.2 se ejecuta contra npm `@mcptoolshop/research-os@0.10.0` en una sesión separada.
- Trabajo sobre la doctrina de admisibilidad. En espera de la aprobación de la versión 0.2.
- Una mejora sobre las herramientas de investigación basadas en la nube.
- Un modelo de calibración completo para revisores de confianza.

La versión v0.10.0 es un requisito previo para la versión 0.2 de la puerta de "trabajo individual del operador", no una prueba de su funcionamiento.

Consulte [`docs/release-notes/v0.10.0.md`](docs/release-notes/v0.10.0.md) y [CHANGELOG.md](CHANGELOG.md).

## Anteriormente: v0.9.0 — Arco de artefactos del producto

La versión v0.9.0 convirtió la columna de evidencia de la versión v0.8 en artefactos útiles para los operadores: síntesis de texto a nivel de sección (`synth section`), síntesis parcial del paquete (`synth pack --partial`) y el sistema de asesoramiento de recuperación legal (`recover pack`). Consulte [`docs/release-notes/v0.9.0.md`](docs/release-notes/v0.9.0.md).
...
**v0.10.0 — Lanzamiento de corrección de errores relacionados con la operación individual** — publicado en npm como `@mcptoolshop/research-os@0.10.0`, 15 de mayo de 2026. La versión v0.10.0 corrige las condiciones de fallo del "gate" de operación individual de la versión v0.1 (`operator_aloneness_dst_v0.1`, FALLO el 15 de mayo de 2026) mediante un proceso de corrección de 4 pasos. **R-001** (`research-os claim repair-scope <sección> [--auto | --interactive]`): Nueva interfaz de línea de comandos para corregir reclamaciones cuyo campo `scope` llegó como `null` durante la extracción; registro de solo escritura `evidence/claim-scope-repairs.jsonl`; nueva acción `repair_claim_scope` en `RECOVERY_ACTIONS` (el enum ampliado de 7 a 8 elementos); el sistema muestra esto como la opción de mayor prioridad en `accepted_claim_floor` cuando hay ≥3 reclamaciones en `needs_repair_claims`. **R-002** (enrutamiento de recuperación): La capa de diagnóstico ahora lee `gate.json:blocking_reasons[]` como la fuente de información autorizada para el enrutamiento, antes de recurrir a la búsqueda heredada `failures[].check`; las señales que bloquean el "gate" tienen prioridad sobre las señales posteriores, como `source_card_classification_gap`. **R-003 + R-005** (endurecimiento de la auditoría de las tarjetas de origen, combinados): Nuevas severidades: `bot_check_or_captcha_detected` (FALLO GRAVE — señal compuesta: marcadores + forma del cuerpo) y `extraction_suspect_word_count_mismatch` (ADVERTENCIA Y CUARENTENA — cuerpo ≤200 palabras Y extraído ≥800 palabras Y ratio ≥4). Anulación por parte del operador a través del nuevo campo `clear_severities[]` en el esquema del registro de anulaciones de la versión v0.4. Bloque opcional `audit.severity_thresholds` en `research.yaml` para ajuste específico de cada paquete. **R-004** (`gather_outcome` preciso): Enum de 5 valores en `FetchReceipt` (`ok | fetch_failed | extraction_skipped | extraction_failed | bot_check_detected`); la frase confusa de la versión v0.1 `"Failed (ok HTTP 200)"` ya no existe. Duración opcional de la recuperación en milisegundos (`fetch_duration_ms`) en `FetchReceipt` para la señal de desafío rápido de CDN de R-003. `BOT_CHECK_MARKERS` exportado desde `src/sources/severities.ts` para la reutilización de marcadores de única fuente de verdad en la capa de recopilación y la capa de auditoría. **Requiere `ollama-intern-mcp@^2.4.0`** (sin cambios desde la versión v0.8.0). 1344/1344 pruebas de vitest superadas (de 1266 a 1344, +78 pruebas en total). **Los cuatro paquetes "frozen" verifican la identidad de bytes con las versiones base de v0.3.3** (séptimo lanzamiento consecutivo). **No es una versión v1. No es una decisión sobre el "gate" de operación individual de la versión v0.2** — la versión v0.2 se ejecuta contra esta versión de npm en una sesión separada. El trabajo sobre la doctrina de admisibilidad está condicionado a que la versión v0.2 supere las pruebas. Consulte [`docs/release-notes/v0.10.0.md`](docs/release-notes/v0.10.0.md) y [CHANGELOG.md](CHANGELOG.md).
...
### Lo que research-os no es (y lo que la versión v0.10.0 no pretende ser)

- No se ha comprobado la independencia del operador en paquetes nuevos. La versión v0.10.0 soluciona las condiciones de fallo de la puerta "operator-aloneness"; la versión v0.2 de la puerta "operator-aloneness" se probará en una sesión separada y podría revelar más correcciones. La versión v0.10.0 es un requisito previo para la versión v0.2, no una prueba.
- No ha sido probado por usuarios externos más allá de las pruebas internas. Se completaron seis experimentos internos, uno de referencia, cinco en dominios externos (ComfyUI, XRPL, Godot, calibración de revisores, revisor determinista), pero el uso a gran escala por parte de operadores externos sigue siendo un trabajo futuro.
- No es un generador de paquetes completos. La versión v0.10.0 hereda las funciones de ámbito de sección (`synth section`) y de paquete parcial (`synth pack --partial`) de la versión v0.9, cada una con una declaración explícita de la preparación del paquete. La síntesis de paquetes completos aún requiere un paquete con la etiqueta `synthesis_ready` y la creación de contenido por parte de un autor humano (o de un colaborador) utilizando los ID de reclamación aceptados a través de `synth workspace`.
- No es una aprobación de ningún modelo de revisor. La versión v0.10.0 no incluye un perfil de revisor "trusted_baseline" de forma predeterminada; los recibos de calibración son evidencia, no una aprobación. Los recibos de calibración existentes de la versión v0.6.0 son anteriores a la arquitectura MCP de la versión v0.8.0 y no se han vuelto a calibrar bajo la ruta MCP. Consulte la [página del manual de calibración de revisores](https://mcp-tool-shop-org.github.io/research-os/handbook/reviewer-calibration/).
- No está libre de artefactos históricos en paquetes congelados. Los paquetes congelados anteriores a la versión v0.4 contienen `research_os_version: '0.1.0'` debido a una constante de estructura codificada en versiones anteriores a la v0.4; la corrección se implementó en la versión v0.4.0, pero los paquetes congelados anteriores son inmutables según la Ley 15 (consulte [`handbook/known-limitations`](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/)).
- No tiene una certificación de origen en npm. La certificación de origen de Sigstore se pospone a una versión futura; verifique los paquetes npm de la versión v0.10.0 mediante package-shasum y el commit de la versión de GitHub.
- No es una mejora significativa en comparación con la infraestructura en la nube. La prueba del producto en `local-first-vs-cloud-research/` de la versión v0.7.x identificó las ventajas de la nube en términos de legibilidad y carga de trabajo del operador; la versión v0.10.0 no afirma que estas ventajas se hayan superado.

### Limitaciones conocidas

La versión v0.10.0 incluye tres limitaciones conocidas visibles para los operadores, heredadas de versiones anteriores. Cada una está documentada en la [página de limitaciones conocidas del manual](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/) y en [CHANGELOG.md](CHANGELOG.md). Ninguna impide la publicación; todas tienen una ruta de recuperación o mitigación definida.

- **B-E-001: La versión de la estructura de los paquetes congelados anteriores a la v0.4 es un artefacto histórico.** Los paquetes congelados publicados entre las versiones v0.3.3 y v0.6.0 contienen `research_os_version: "0.1.0"` en `pack.manifest.json` y `pack/research.yaml` debido a una constante de estructura codificada en versiones anteriores a la v0.4. La corrección se implementó en la versión v0.4.0 (la estructura ahora importa la versión `RESEARCH_OS_VERSION` actual); los paquetes congelados anteriores son inmutables según la Ley 15. Los archivos JSON dentro de los paquetes afectados ya contienen sus versiones correspondientes.
- **B-E-004: La certificación de origen de npm se pospone a una versión futura.** La versión v0.10.0 verifica el archivo tarball de npm solo mediante package-shasum. La migración del flujo de publicación a un flujo de trabajo de CI con OIDC de Sigstore entra en conflicto con la disciplina de "traducir antes de publicar" (TranslateGemma 12B se ejecuta localmente); la migración está planificada para una versión futura. Verifique los paquetes npm de la versión v0.10.0 mediante package-shasum y el commit de la versión de GitHub.
- **B-A-003: La migración del esquema de la versión del indexador está documentada, no impuesta.** La versión v0.10.0 incluye un entero `SCHEMA_VERSION` para la escritura, pero no un ejecutor de migración de lectura. Cuando se actualiza la `SCHEMA_VERSION` según la documentación, elimine `.research-os/index.sqlite` y vuelva a ejecutar `research-os index build --all`. El propio paquete no se ve afectado; el indexador es una capa de aceleración sobre la evidencia y las reclamaciones (Ley 8); la reconstrucción es idempotente.

**En la versión 0.10.0, no se admite ningún perfil de revisor de "línea de base confiable".** Esto es una decisión deliberada en cuanto a la confianza, no una deficiencia: los registros de calibración en el repositorio (`hermes-two-pass=failed`, `mistral-nemo-two-pass=conditional_pass`, `hermes-single-pass=comparison_only`, `hermes-two-pass-deterministic=failed`) registran la evidencia. La confianza se gana a través de pruebas repetidas de fallos controlados, no se asume. Estos registros son anteriores a la arquitectura MCP de la versión 0.8.0 y no se han vuelto a calibrar bajo el camino MCP.
