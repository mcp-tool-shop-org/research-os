...
**Requiere [`ollama-intern-mcp@^2.4.0`](https://github.com/mcp-tool-shop-org/ollama-intern-mcp) en ejecución local** para la extracción, clasificación, revisión y descubrimiento de modelos de lenguaje (LLM). El servidor MCP se descubre a través de la variable de entorno `OLLAMA_INTERN_MCP_BIN` o PATH. El modelo predeterminado es `hermes3:8b`; puede modificarse con `OLLAMA_INTERN_MODEL=<modelo>` (o con la opción `--model <nombre>` en cada llamada). Establezca `OLLAMA_HOST` si Ollama no está en la ubicación predeterminada `localhost:11434`.
...
## Novedades en la versión 0.8.0

La versión 0.8.0 es una versión de recuperación de la arquitectura: research-os ahora utiliza `ollama-intern-mcp@^2.4.0` como la base local para el procesamiento de evidencia, que se utiliza para la extracción de afirmaciones, añade la aplicación de la relevancia dentro de los marcos, conserva la evidencia que no es relevante como excluida en lugar de utilizable, y añade la síntesis de evidencia a nivel de sección para las secciones elegibles para la validación en los paquetes que requieren corrección.

Consulte [`docs/release-notes/v0.8.0.md`](docs/release-notes/v0.8.0.md) y [CHANGELOG.md](CHANGELOG.md).

## Estado

**v0.8.0 — Recuperación de la arquitectura + Relevancia dentro de los marcos** — publicada en npm como `@mcptoolshop/research-os@0.8.0`, 12 de mayo de 2026. La versión 0.8.0 es una versión de recuperación de la arquitectura: research-os ahora utiliza `ollama-intern-mcp@^2.4.0` como la base local para el procesamiento de evidencia, que se utiliza para la extracción de afirmaciones (anteriormente, el README declaraba la dependencia, pero el código tenía implementaciones internas directas de Ollama que la omitían desde el esqueleto de la versión 0.1; la versión 0.8.0 corrige esta situación). Se añaden: la base de cliente MCP (`OLLAMA_INTERN_MCP_BIN` variable de entorno + descubrimiento de PATH + transporte StdioClient); la evaluación de evidencia a nivel de sección para cada afirmación a través de `ollama_extract` con un esquema de 4 etiquetas (`supports_section` / `off_topic` / `background_only` / `source_chrome`); la nueva decisión de revisión `frame_excluded` (la revisión omite el LLM para las afirmaciones excluidas, generando un `ClaimReview` sintético); el esquema de afirmación (`ClaimSchema`) incluye `frame_excluded` + `frame_exclusion_reason` (una enumeración de 4 valores, incluyendo `critic_unavailable` para fallos en el estado del sistema) + `frame_exclusion_rationale`; la síntesis de evidencia a nivel de sección a través de `synth section <id>` para las secciones elegibles para la validación en los paquetes que requieren corrección (índice de citas de evidencia: ID de la afirmación → aserción → extracto de evidencia → URL de origen — NO texto narrativo); la validación respeta el registro de anulación de la tarjeta de origen a través de `getEffectivePublisher` / `getEffectiveSourceType` (absorbe el objetivo de la versión 0.7.1); el valor predeterminado de `DEFAULT_WINDOW_CHARS` se cambia de 5000 a 3000 (tamaño adecuado para hermes3:8b con un contexto de trabajo de 8K en el perfil `dev-rtx5080`); la política de fallo suave en la llamada al evaluador se invierte (cualquiera de los 5 modos de fallo — transporte / análisis / etiqueta no válida / justificación vacía / tiempo de espera — por defecto, se establece en `frame_excluded: true` con la razón `critic_unavailable`, en lugar de admitir); la semántica de promoción: las afirmaciones `frame_excluded` no bloquean la promoción de la sección; la transferencia de trabajo muestra `frame_excluded` como un bucket separado de los aceptados / que requieren corrección / rechazados. **Requiere `ollama-intern-mcp@^2.4.0`**. 1013 de 1013 pruebas de vitest superadas (de 901 a 1013, +112 pruebas). **Los cuatro paquetes congelados verifican de forma idéntica en bytes con las versiones base de la versión 0.3.3.** **No es una versión 1** — el trabajo para la preparación de la versión 1 continúa; consulte [`docs/roadmap.md`](docs/roadmap.md). Consulte [`docs/release-notes/v0.8.0.md`](docs/release-notes/v0.8.0.md) y [CHANGELOG.md](CHANGELOG.md).
...
- No ha sido probado en entornos reales por usuarios externos, más allá de las pruebas internas. Se han cerrado seis experimentos de prueba interna, uno de referencia interna y cinco relacionados con áreas externas (ComfyUI, XRPL, Godot, calibración de revisores, revisor determinista), pero el uso a gran escala por parte de operadores externos es un trabajo futuro.
- No es un generador de texto. El comando `synth workspace` genera el espacio de trabajo estructurado; los humanos (o Cowork) escriben el texto en función de los identificadores de reclamación aceptados.
- No es una aprobación de ningún modelo de revisor. La versión 1.0 no incluye un perfil de revisor "de referencia" por defecto; los registros de calibración son evidencia, no una aprobación. Consulte la [página del manual de calibración de revisores](https://mcp-tool-shop-org.github.io/research-os/handbook/reviewer-calibration/).
- No está libre de artefactos históricos en los paquetes congelados. Los paquetes congelados anteriores a la versión 1.0 contienen `research_os_version: '0.1.0'` debido a una marca de plantilla anterior a la versión 0.4; la corrección se implementó, pero los paquetes históricos son inmutables según la Ley 15 (consulte [`handbook/known-limitations`](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/)).
- No tiene una certificación de origen en npm. La certificación de origen de Sigstore se pospone a la versión 1.x; verifique los paquetes npm de la versión 1.0 mediante package-shasum y el commit de la versión en GitHub.
