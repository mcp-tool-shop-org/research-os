...
**v1.0.0** — Publicado en npm como `@mcptoolshop/research-os@1.0.0`, 11 de mayo de 2026. La versión 1.0.0 es una versión estable: el flujo de trabajo está probado, los posibles fallos se han documentado y la síntesis sigue requiriendo evidencia. **research-os no incluye un modelo de revisión confiable de forma predeterminada.** Incluye los mecanismos para probar, rechazar o admitir condicionalmente perfiles de revisores. Incluye: un sistema de pruebas en cuatro etapas (Etapas A: errores/seguridad, B: resiliencia proactiva, C: mejora de la experiencia del usuario, D: pulido de la presentación) que genera 188 nuevas pruebas en 23 archivos de código de producción, +14 archivos de documentación/sitio web, +2 nuevos módulos de utilidad (`src/cli/help-topics.ts`, `src/util/progress.ts`); se ha documentado la doctrina de transferencia entre dominios después de dos correcciones (A-RE-001: migración del llamador, C2-RE-001: recepción entre dominios); la instrucción `--force` se ha anclado byte a byte en 8 áreas con pruebas de regresión; se ha añadido un puntero al manual de errores para cada subclase de `ResearchOSError`; la función `research-os help <topic>` proporciona una interfaz de recuperación (4 temas bloqueados); `--no-progress` / `--progress` detecta el terminal y activa el hilo de progreso durante la revisión/recopilación/comparación/publicación, utilizando semántica de mutex. **No se admite ninguna línea de base confiable.** **Los cuatro paquetes congelados verifican la integridad de los archivos byte a byte en comparación con las líneas de base de la versión 0.3.3.** 901/901 pruebas de vitest superadas. Las limitaciones conocidas se encuentran en [CHANGELOG.md](CHANGELOG.md) y [`handbook/known-limitations.md`](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/): la versión congelada es un artefacto histórico (B-E-001), la certificación de origen de npm se pospone a la versión 1.x (B-E-004), la migración del esquema de índice se basa en la divulgación, no en la migración automática (B-A-003). Consulte [`docs/release-notes/v1.0.0.md`](docs/release-notes/v1.0.0.md) y [`docs/v1-dogfood-swarm-proof.md`](docs/v1-dogfood-swarm-proof.md).
