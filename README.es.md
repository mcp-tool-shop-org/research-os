...
La versión 0.12.1 es un parche único que se aplica a la versión 0.12.0. Incluye únicamente la revisión R-018: un envoltorio en el lado del sistema operativo de investigación que controla el tiempo de espera de las llamadas a `callTool` del MCP de la sintaxis, mediante una opción de línea de comandos que el operador puede descubrir (`--planner-timeout-ms <N>` en `synth section` y `synth workspace`) y una variable de entorno correspondiente (`RESEARCH_OS_SYNTH_PLANNER_TIMEOUT_MS=<N>`). Prioridad: opción de línea de comandos > variable de entorno > valor predeterminado (15000 ms). El comportamiento predeterminado se mantiene idéntico a la versión 0.12.0.

Esta versión se lanzó porque la prueba de "aislamiento del operador" de la versión 0.4 para `@mcptoolshop/research-os@0.12.0` devolvió **"PASS_WITH_CONDITIONS", no una autorización completa** (`operator_aloneness_dst_v0.4`). El nivel de defensa de la versión 0.11 se mantuvo bajo carga; las seis áreas de recuperación de cobertura de la versión 0.12 se activaron y permitieron el acceso del operador; la cobertura del "envelope" alcanzó los umbrales de aprobación (4/5 elementos obligatorios + 1 parcial; 2/3 elementos de moderación + 1 parcial; 0/3 trampas; 0/5 fallos de materiales); todos los indicadores de contaminación fueron inofensivos. El único modo de fallo fue la finalización: la sintaxis alcanzó el límite de `TIER_TIMEOUT` de forma reproducible a los ~15010 ms, superando el presupuesto de 15 segundos para el nivel "instantáneo", sin que el operador haya aplicado ninguna modificación. Los informes de las secciones cumplieron con los requisitos del "envelope"; simplemente, el proceso no pudo completarse.

**Procedimiento de la ruta C** (nuevo patrón obtenido en la versión 0.4): cuando la sesión B identifica un único mecanismo de fallo específico con una ruta de parche explícita Y la cobertura del "envelope" está en los umbrales de aprobación Y el nivel de defensa se mantiene Y la contaminación es inofensiva, el procedimiento es: aplicar el parche, volver a ejecutar la misma ruta del operador en la versión parcheada y volver a evaluar. No se requiere una nueva autorización del "envelope". No hay evaluadores humanos. No hay una nueva rama arquitectónica para la versión 0.13.

> **La versión 0.4 demuestra la calidad de la cobertura del sistema operativo de investigación a nivel de informe de la sección.**
> **La versión 0.12.1 debe demostrar la calidad de la finalización eliminando el cuello de botella del tiempo de espera del planificador sin debilitar el nivel de defensa.**
...
Las configuraciones activas se encuentran en `section-synthesis.json` (`planner_timeout_ms` siempre se completa + `planner_timeout_overridden_by` solo presente cuando se aplica una anulación), en los metadatos de ProseBlock y en stderr (`[synth] planner_timeout_ms=N source=… section=<id>` se muestra antes de la generación de la sintaxis). El comando `synth section --help` documenta la opción, el valor predeterminado, el límite superior (600000 ms como medida de seguridad) y la alternativa de la variable de entorno. Los valores no válidos (negativos, cero, no numéricos, cadenas con sufijos de unidad, > 600000) generan un error claro con un código de salida diferente de cero, indicando la configuración afectada y el valor incorrecto. No hay un valor predeterminado oculto.

### Nota arquitectónica

El presupuesto de 15000 ms que la prueba de la versión 0.4 superó se encuentra en `ollama-intern-mcp` (`profiles.ts:58`, `DEV_RTX5080_TIMEOUTS.instant`), NO en el sistema operativo de investigación. Antes de la revisión R-018, el sistema operativo de investigación no aplicaba ningún tiempo de espera para el planificador; el tiempo de espera se aplicaba en el servidor dentro de la política de nivel de `ollama-intern-mcp`. La revisión R-018 introduce la propia autoridad del sistema operativo de investigación sobre el presupuesto mediante un envoltorio `Promise.race` alrededor de la llamada `callTool` del MCP, con un valor predeterminado que coincide con el número de nivel "instantáneo" observado (15000 ms), lo que mantiene el comportamiento predeterminado. El envoltorio de R-018 genera errores con la forma `TIER_TIMEOUT` que coinciden con la expresión regular `classifyFallbackCause` de la versión R-010 (`/elapsed=(\d+)ms/` + `/budget=(\d+)ms/`), lo que permite que el asesor de IA de nivel inferior tenga visibilidad del comportamiento predeterminado en las ejecuciones de la ruta predeterminada.

### Nivel de defensa mantenido

R-018 es una pequeña modificación en la interfaz de usuario, no un cambio arquitectónico. R-002 / R-003 / R-005 / R-007 / R-008 / R-009 / R-010 / R-011 / R-012 / R-013 / R-014 / R-015 / R-016 / R-017 no se han modificado. El parámetro `accepted_claim_floor` sigue siendo inalterable. Las enumeraciones cerradas no se han modificado (`FailureShape` en 9; `RECOVERY_ACTIONS` en 8; `REGENERATION_REASONS` en 3; `POLICY_KEYWORDS` en 8; `POLICY_RELEVANT_SOURCE_TYPES` en 1). La plantilla de solicitud del asesor de recuperación con IA no se ha modificado. La arquitectura de MCP no se ha modificado; `ollama-intern-mcp@^2.4.0` se mantiene. R-018 añade `PLANNER_TIMEOUT_SOURCES` (3) como nuevo vocabulario de registro del operador, distinto de cualquier enumeración de enrutamiento.

La regresión de los paquetes congelados es idéntica en bytes a las versiones base v0.3.3 para los cuatro paquetes congelados; esta es la **décimo sexta versión consecutiva** en la que esto se cumple. 1542 → 1586 pruebas de vitest superadas (+44 pruebas de aceptación de R-018).

### Lo que la versión v0.12.1 NO afirma:

- Preparación para la versión 1.
- Verificación de la ejecución de la puerta "operator-aloneness" para la versión 0.4. La ejecución de la versión 0.4 se realiza contra `@mcptoolshop/research-os@0.12.1` en una sesión separada; la versión 0.12.1 es un requisito previo para la finalización, no una prueba.
- Admisión de la sección 1. Depende de la aprobación de la ejecución de la versión 0.4; el "ratchet" de la versión 0.4 (aislamiento probado a nivel de defensa; aislamiento probado sustancialmente a nivel de resumen de sección; la finalización está pendiente de la versión 0.12.1) sigue siendo la prueba bloqueada.
- Candidatos para la versión 0.13 (F-2: divergencia de auditoría↔extracción R-009; F-3: obsolescencia de la transferencia de trabajo colaborativo; F-4: limitación de `POLICY_KEYWORDS` en R-017). Independiente de la finalización.

Consulte [CHANGELOG.md](CHANGELOG.md) para ver la entrada completa de la versión.

## Anteriormente: v0.12.0 — Versión de Recuperación y Cobertura
...
### Lo que research-os no es (y lo que la versión v0.12.1 no afirma ser)

- No se ha comprobado la independencia del operador en paquetes nuevos. La versión 0.12.0 resolvió los problemas identificados en la fase de pruebas 0.3 (se comprobó la independencia del operador a nivel de seguridad; la independencia a nivel de cobertura aún no se ha verificado, y se aplicó la mejora correspondiente en la versión 0.3). La nueva prueba de la fase 0.4 contra la versión 0.12.0 obtuvo un resultado de "PASADO CON CONDICIONES" (no cumple con los requisitos de autorización), pero se mantuvo el nivel de seguridad básico, y la independencia a nivel de cobertura se comprobó sustancialmente a nivel de resumen de sección, con un único modo de fallo en la finalización. La versión 0.12.1 corrige ese único modo de fallo (R-018). La nueva ejecución de la fase 0.4 contra esta versión de npm se realiza en una sesión separada y es un requisito previo para la finalización.
- No ha sido probado en condiciones reales por usuarios externos, más allá de las pruebas internas y las cuatro pruebas de independencia del operador. Se completaron seis experimentos internos, uno de referencia y cinco relacionados con dominios externos (ComfyUI, XRPL, Godot, calibración del revisor, revisor determinista), además de las pruebas de independencia del operador de las versiones 0.1, 0.2, 0.3 y 0.4, que revelaron 18 problemas identificados (R-001 a R-005 resueltos en la versión 0.10.0, R-007 a R-011 resueltos en la versión 0.11.0, R-012 a R-017 resueltos en la versión 0.12.0, R-018 resuelto en la versión 0.12.1). El uso del operador a gran escala sigue siendo un trabajo futuro.
- No es un generador completo de paquetes. La versión 0.12.1 hereda las funciones de ámbito de sección (`synth section`) y de ámbito de paquete parcial (`synth pack --partial`) de la versión 0.9, cada una con una declaración explícita de la preparación del paquete. La generación completa de paquetes aún requiere un paquete con la etiqueta `synthesis_ready` y la creación manual (o con Cowork) basada en los ID de reclamación aceptados a través de `synth workspace`.
- No es una aprobación de ningún modelo de revisor. La versión 0.12.1 no incluye un perfil de revisor predeterminado llamado `trusted_baseline`; los recibos de calibración son evidencia, no aprobación. Los recibos de calibración existentes de la versión 0.6.0 son anteriores a la arquitectura MCP de la versión 0.8.0 y no se han vuelto a calibrar bajo la ruta MCP. Consulte la [página del manual de calibración del revisor](https://mcp-tool-shop-org.github.io/research-os/handbook/reviewer-calibration/).
- No está libre de artefactos históricos en los paquetes congelados. Los paquetes congelados anteriores a la versión 0.4 contienen `research_os_version: '0.1.0'` debido a una constante predefinida anterior a la versión 0.4; la corrección se implementó en la versión 0.4.0, pero los paquetes congelados anteriores son inmutables según la Ley 15 (consulte [`handbook/known-limitations`](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/)).
- No tiene una certificación de origen en npm. La certificación de origen de Sigstore se pospone a una versión futura; verifique los paquetes npm de la versión 0.12.1 mediante package-shasum y el commit de la versión en GitHub.
- No representa una mejora significativa en comparación con las soluciones basadas en la nube. La demostración del producto en `local-first-vs-cloud-research/` de la versión 0.7.x identificó las ventajas de la nube en términos de legibilidad y carga de trabajo del operador; la versión 0.12.1 no afirma que estas ventajas se hayan superado.

### Limitaciones conocidas

La versión 0.12.1 incluye tres limitaciones conocidas visibles para el operador, heredadas de versiones anteriores. Cada una está documentada en la [página de limitaciones conocidas del manual](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/) y en [CHANGELOG.md](CHANGELOG.md). Ninguna impide la publicación; todas tienen una ruta de recuperación o mitigación definida.

- **B-E-001 — La marca de versión "frozen-pack" pre-v0.4 es un artefacto histórico.** Los paquetes "frozen" publicados entre las versiones v0.3.3 y v0.6.0 contienen `research_os_version: "0.1.0"` en `pack.manifest.json` y `pack/research.yaml` debido a una constante predefinida anterior a la versión v0.4. La corrección se implementó en la versión v0.4.0 (ahora el "scaffold" importa la versión activa de `RESEARCH_OS_VERSION`); los paquetes "frozen" anteriores son inmutables según la Ley 15. Los archivos JSON de auditoría dentro de los paquetes afectados ya contienen sus versiones correspondientes.
- **B-E-004 — La verificación de la procedencia de npm se pospone a una futura versión.** El archivo "tarball" de npm v0.12.1 verifica únicamente mediante "package-shasum". La migración del flujo de publicación a un flujo de CI con OIDC de Sigstore entra en conflicto con la práctica de "traducir antes de publicar" (TranslateGemma 12B se ejecuta localmente); la migración está planificada para una futura versión. Verifique los paquetes npm v0.12.1 mediante "package-shasum" y el commit de la versión en GitHub.
- **B-A-003 — La migración del esquema de la versión del indexador está documentada, pero no se aplica obligatoriamente.** La versión v0.12.1 incluye un entero `SCHEMA_VERSION` para la escritura, pero no un ejecutor de migración para la lectura. Cuando se actualiza la versión del esquema según la documentación, elimine `.research-os/index.sqlite` y vuelva a ejecutar `research-os index build --all`. El paquete en sí no se ve afectado; el indexador es una capa de aceleración sobre las evidencias y las afirmaciones (Ley 8); la reconstrucción es idempotente.

**No se admite ningún perfil de revisor de "trusted_baseline" en la versión v0.12.1.** Esto es una postura de confianza intencionada, no una deficiencia: los recibos de calibración en el repositorio (`hermes-two-pass=failed`, `mistral-nemo-two-pass=conditional_pass`, `hermes-single-pass=comparison_only`, `hermes-two-pass-deterministic=failed`) registran la evidencia. La confianza se gana a través de la recuperación repetida de fallos simulados, no se asume. Estos recibos son anteriores a la arquitectura MCP de la versión v0.8.0 y no se han vuelto a calibrar bajo la ruta MCP.
