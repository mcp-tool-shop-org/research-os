...
## Nouvelle version v0.10.0 — Version de correction des problèmes liés à l'absence d'opérateur

La version v0.10.0 corrige les conditions d'échec de la porte "operator-aloneness" de la version v0.1, signalées le 15 mai 2026 (`operator_aloneness_dst_v0.1`, ÉCHEC). Quatre améliorations sont intégrées : alignement de la redirection de récupération, outil en ligne de commande (CLI) de correction de la portée, renforcement de l'audit des cartes sources et affichage précis de l'état. La porte v0.1 a échoué car un opérateur externe a rencontré trois problèmes indépendants : `recover` a recommandé un chemin de déblocage incorrect, aucun outil en ligne de commande n'existait pour corriger les affirmations avec `scope=null`, et l'audit de la carte source a validé une carte COVID-19 falsifiée extraite d'un fragment de 1035 octets provenant d'un bot-check Incapsula. La version v0.10.0 résout ces problèmes.
...
Avant l'extraction des affirmations, l'audit de la carte source isole désormais les fragments de vérification de bot / CAPTCHA (`bot_check_or_captcha_detected`, ÉCHEC GRAVE) et les sources de faible qualité / à forte extraction (`extraction_suspect_word_count_mismatch`, AVERTISSEMENT ET ISOLEMENT). Les opérateurs peuvent outrepasser ces mesures via le nouveau champ `clear_severities[]` dans le registre de remplacement de la carte source de la version v0.4.

L'état d'avancement de la collecte affiche désormais une énumération `gather_outcome` avec 5 valeurs (`ok | fetch_failed | extraction_skipped | extraction_failed | bot_check_detected`) dans chaque reçu de récupération. La phrase trompeuse de la version v0.1, "Échec (HTTP 200 OK)", a été supprimée ; les fichiers PDF sont désormais signalés comme `extraction_skipped` et non comme "Échec".

### Limites des règles

L'amélioration apportée est incrémentale. Les règles de base sont conservées : `accepted_claim_floor` reste immuable ; le conseiller de récupération continue de refuser de recommander `apply_waiver` en cas d'échec immuable. L'énumération `FailureShape` fermée n'a pas été modifiée ; R-002 ajoute uniquement le routage de l'état de la porte aux neuf formes existantes. `RECOVERY_ACTIONS` passe de 7 à 8 valeurs fermées (ajout de `repair_claim_scope`). L'isolement par gravité n'entraîne jamais une promotion automatique au-delà de la porte d'audit sans intervention explicite de l'opérateur (le nouveau champ `clear_severities[]` représente une décision de l'opérateur, enregistrée et modifiable uniquement).

La régression du paquet figé est identique en octets aux versions de base v0.3.3 pour les quatre paquets figés, ce qui est le cas pour la septième version consécutive.

### Ce que la version v0.10.0 NE prétend PAS

- Préparation pour la version 1.
- Verdict de la porte "operator-aloneness" pour la version 0.2. La version 0.2 est exécutée dans une session distincte avec npm `@mcptoolshop/research-os@0.10.0`.
- Travaux sur la doctrine d'admissibilité. En attente du passage de la version 0.2.
- Une victoire sur les outils de recherche basés sur le cloud.
- Un modèle de calibration complet pour les examinateurs.

La version v0.10.0 est une condition préalable à la version 0.2 de la porte "operator-aloneness", et non une preuve de son fonctionnement.

Consultez les documents [`docs/release-notes/v0.10.0.md`](docs/release-notes/v0.10.0.md) et [CHANGELOG.md](CHANGELOG.md).
...
**v0.10.0 — Version corrigeant les problèmes d'isolement des opérateurs** — publiée sur npm sous le nom `@mcptoolshop/research-os@0.10.0`, le 15 mai 2026. La version v0.10.0 corrige les conditions d'erreur du mécanisme de "seul opérateur" de la version v0.1 (`operator_aloneness_dst_v0.1`, ERREUR le 15 mai 2026) grâce à une correction en quatre étapes. **R-001** (`research-os claim repair-scope <section> [--auto | --interactive]`): Nouvelle interface en ligne de commande pour corriger les demandes dont le champ `scope` est revenu à `null` après l'extraction ; registre en écriture seule `evidence/claim-scope-repairs.jsonl`; nouvelle action `repair_claim_scope` dans `RECOVERY_ACTIONS` (l'énumération s'agrandit de 7 à 8 éléments); le système d'aide affiche cette action comme étant de priorité 1 dans `accepted_claim_floor` lorsque ≥3 demandes nécessitent une correction. **R-002** (routage de la récupération): la couche de diagnostic lit désormais `gate.json:blocking_reasons[]` comme la source d'information principale pour le routage, avant de revenir à la méthode traditionnelle `failures[].check` — les signaux bloquant le processus ont priorité sur les signaux provenant des étapes suivantes, comme `source_card_classification_gap`. **R-003 + R-005** (renforcement de l'audit des sources, en paire): nouvelles sévérités `bot_check_or_captcha_detected` (ERREUR FATALE — signal combiné : marqueurs + forme du contenu) et `extraction_suspect_word_count_mismatch` (AVERTISSEMENT ET MISE EN QUARANTÂINE — contenu ≤200 mots ET nombre de mots extraits ≥800 mots ET ratio ≥4). Possibilité de remplacer les paramètres par l'opérateur via le nouveau champ `clear_severities[]` dans le schéma du registre de remplacement de la version v0.4. Bloc `audit.severity_thresholds` optionnel dans `research.yaml` pour un réglage spécifique à chaque paquet. **R-004** (`gather_outcome` fiable): Énumération à 5 valeurs pour `FetchReceipt` (`ok | fetch_failed | extraction_skipped | extraction_failed | bot_check_detected`); la phrase trompeuse de la version v0.1 `"Failed (ok HTTP 200)"` a été supprimée. `fetch_duration_ms` optionnel dans `FetchReceipt` pour le signal de défi rapide CDN de R-003. `BOT_CHECK_MARKERS` exporté depuis `src/sources/severities.ts` pour la réutilisation des marqueurs dans la couche de collecte et la couche d'audit. **Nécessite `ollama-intern-mcp@^2.4.0`** (inchangé depuis la version v0.8.0). 1344/1344 tests vitest réussis (1266 → 1344, +78 tests au total). **Les quatre paquets figés sont vérifiés de manière identique en termes de bytes par rapport aux versions de base v0.3.3** (septième version consécutive). **Ce n'est pas une version v1. Ce n'est pas non plus un verdict sur le mécanisme de "seul opérateur" de la version v0.2** — les tests de la version v0.2 sont effectués sur cette version npm dans une session distincte. Le travail sur la doctrine d'admissibilité est conditionné au succès de la version v0.2. Consultez [`docs/release-notes/v0.10.0.md`](docs/release-notes/v0.10.0.md) et [CHANGELOG.md](CHANGELOG.md).
...
### Ce que research-os n'est pas (et ce que la version v0.10.0 ne prétend pas être)

- Non prouvé que l'opérateur fonctionne de manière autonome sur les packs récents. La version v0.10.0 corrige les conditions d'erreur de la fonctionnalité "operator-aloneness" (v0.1). La version v0.2 de cette même fonctionnalité sera testée dans une session distincte et pourrait révéler d'autres corrections. La version v0.10.0 est une condition préalable à la version v0.2, et non une preuve de son fonctionnement.
- Non testé par des utilisateurs externes au-delà des phases de test interne. Six expériences de test interne ont été réalisées, dont une auto-référentielle et cinq portant sur des domaines externes (ComfyUI, XRPL, Godot, calibrage des évaluateurs, évaluation déterministe), mais l'utilisation de l'opérateur à grande échelle reste un objectif futur.
- N'est pas un outil de création de packs complets. La version v0.10.0 hérite des fonctionnalités de la version v0.9 concernant la portée des sections (`synth section`) et des packs partiels (`synth pack --partial`), avec une indication explicite de la préparation du pack. La création de packs complets nécessite toujours un pack "synthesis_ready" et une création par un utilisateur (ou un collaborateur) en utilisant les identifiants de revendications acceptés via `synth workspace`.
- Ne constitue pas une approbation de modèle d'évaluateur. La version v0.10.0 ne fournit pas de profil d'évaluateur "trusted_baseline" par défaut ; les reçus de calibrage sont des preuves, et non des approbations. Les reçus de calibrage existants de la version v0.6.0 datent d'avant l'architecture MCP de la version v0.8.0 et n'ont pas été réinitialisés dans le cadre de cette architecture. Consultez la [page du manuel de calibrage des évaluateurs](https://mcp-tool-shop-org.github.io/research-os/handbook/reviewer-calibration/).
- Contient des artefacts historiques dans les packs figés. Les packs figés antérieurs à la version v0.4 contiennent `research_os_version: '0.1.0'` en raison d'une constante de structure codée en dur antérieure à la version v0.4 ; cette correction a été intégrée dans la version v0.4.0, mais les packs figés antérieurs sont immuables conformément à la loi 15 (voir [`handbook/known-limitations`](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/)).
- La provenance n'est pas attestée sur npm. L'attestation de provenance Sigstore est reportée à une version ultérieure ; vérifiez les packages npm de la version v0.10.0 via package-shasum et le commit de la version sur GitHub.
- Ne représente pas une amélioration significative par rapport à une solution basée sur le cloud. L'étude comparative entre une solution locale et une solution basée sur le cloud, réalisée dans le cadre du projet `local-first-vs-cloud-research/` (version v0.7.x), a identifié les avantages du cloud en termes de lisibilité et de charge de travail pour l'opérateur ; la version v0.10.0 ne prétend pas avoir résolu ces problèmes.

### Limitations connues

La version v0.10.0 est livrée avec trois limitations connues, visibles pour l'opérateur, qui ont été conservées des versions précédentes. Chacune est documentée dans la [page du manuel sur les limitations connues](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/) et dans le fichier [CHANGELOG.md](CHANGELOG.md). Aucune de ces limitations n'empêche la publication ; chacune a une méthode de récupération ou d'atténuation définie.

- **B-E-001 — La version du pack figé antérieure à la version v0.4 est un artefact historique.** Les packs figés publiés sous les versions v0.3.3 à v0.6.0 contiennent `research_os_version: "0.1.0"` dans les fichiers `pack.manifest.json` et `pack/research.yaml` en raison d'une constante de structure codée en dur antérieure à la version v0.4. Cette correction a été intégrée dans la version v0.4.0 (la structure importe désormais la version `RESEARCH_OS_VERSION` en direct) ; les packs figés antérieurs sont immuables conformément à la loi 15. Les fichiers JSON à l'intérieur des packs concernés contiennent déjà leurs versions contemporaines.
- **B-E-004 — L'attestation de provenance npm est reportée à une version ultérieure.** La version v0.10.0 vérifie les archives tar npm uniquement via package-shasum. La migration du flux de publication vers un workflow CI avec sigstore OIDC est incompatible avec la discipline de "traduction avant publication" (TranslateGemma 12B s'exécute localement) ; cette migration est prévue pour une version ultérieure. Vérifiez les packages npm de la version v0.10.0 via package-shasum et le commit de la version sur GitHub.
- **B-A-003 — La migration du schéma de l'index est documentée, mais n'est pas appliquée de force.** La version v0.10.0 inclut un entier `SCHEMA_VERSION` côté écriture, mais pas de programme de migration côté lecture. Lors d'une mise à jour documentée de `SCHEMA_VERSION`, supprimez `.research-os/index.sqlite` et relancez `research-os index build --all`. Le pack lui-même n'est pas affecté ; l'index est une couche d'accélération au-dessus des preuves et des revendications (Loi 8) ; la reconstruction est idempotente.

**Aucun profil de réviseur "trusted_baseline" n'est accepté dans la version 0.10.0.** Il s'agit d'une décision délibérée en matière de confiance, et non d'une lacune : les rapports de calibration enregistrés dans le référentiel (`hermes-two-pass=échec`, `mistral-nemo-two-pass=succès conditionnel`, `hermes-single-pass=comparaison uniquement`, `hermes-two-pass-deterministic=échec`) fournissent les preuves. La confiance se gagne par des tests répétés et contrôlés, et non par une simple hypothèse. Ces rapports datent de l'architecture MCP (Master Control Plane) de la version 0.8.0 et n'ont pas été réévalués dans le cadre du chemin MCP.
