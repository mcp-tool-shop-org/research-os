...
- Não foi testado por usuários externos. Dois ciclos de testes internos foram concluídos — um de referência interna e outro com um domínio externo — e o Experimento 3 (estabilidade da API sob pressão externa) está em andamento: o pacote nº 1 de 3 (durabilidade do token criador do XRPL) obteve tanto a flag `--detector` da versão v0.3.0 quanto as isenções de direitos autorais de seção da versão v0.3.1. São necessários mais dois pacotes de domínio externo para a conclusão do Experimento 3.
- Não é um gerador de conteúdo. O comando `synth workspace` gera o ambiente de trabalho estruturado; humanos (ou Cowork) escrevem o conteúdo com base nos IDs de reivindicação aprovados.
- Não é estável em termos de API, seguindo a semântica de versionamento. A versão v1.0.0 é um estado alcançado, não uma data no calendário — consulte [`docs/roadmap.md`](docs/roadmap.md) para os seis experimentos que preenchem essa lacuna.
