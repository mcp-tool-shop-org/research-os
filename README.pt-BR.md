...
**v1.0.0** — publicado no npm como `@mcptoolshop/research-os@1.0.0`, em 11 de maio de 2026. A versão 1.0.0 é uma versão que define um contrato: o fluxo de trabalho é comprovado, os modos de falha são divulgados e a síntese continua dependente de evidências. **O `research-os` não inclui, por padrão, um modelo de revisor confiável.** Ele fornece os mecanismos para provar, rejeitar ou admitir condicionalmente perfis de revisores. Inclui: um conjunto de testes de quatro etapas (Etapas A: correção de bugs/segurança, B: resiliência proativa, C: otimização da experiência do usuário, D: refinamento da apresentação) que geram 188 novos testes em 23 arquivos de código de produção, +14 arquivos de documentação/site, +2 novos módulos auxiliares (`src/cli/help-topics.ts`, `src/util/progress.ts`); a doutrina de transferência entre domínios foi formalizada após duas correções de "escapes" (A-RE-001: migração do chamador, C2-RE-001: coleta entre domínios); a frase canônica `--force` foi fixada byte a byte em 8 áreas, com um teste de regressão; retrofitting de ponteiros para manuais de erro em 12 subclasses de `ResearchOSError`; superfície de recuperação estática `research-os help <tópico>` (4 tópicos bloqueados); threading TTY-detect para `--no-progress` / `--progress` em revisão/coleta/mapeamento de contradições/publicação de pacotes, com semântica de mutex. **Nenhuma linha de base confiável foi admitida.** **Todos os quatro pacotes congelados verificam a identidade dos bytes em relação às linhas de base da versão 0.3.3.** 901/901 testes vitest passaram. As limitações conhecidas estão documentadas em [CHANGELOG.md](CHANGELOG.md) e [`handbook/known-limitations.md`](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/): o artefato histórico de versão do pacote congelado (B-E-001), a atestação de origem do npm foi adiada para a versão 1.x (B-E-004), a migração do esquema de índice é baseada em divulgação, não em migração automática (B-A-003). Consulte [`docs/release-notes/v1.0.0.md`](docs/release-notes/v1.0.0.md) e [`docs/v1-dogfood-swarm-proof.md`](docs/v1-dogfood-swarm-proof.md).
