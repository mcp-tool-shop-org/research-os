...
**v1.0.0** — published to npm as `@mcptoolshop/research-os@1.0.0`, 2026-05-11. v1.0.0 is an honest contract release: the workflow is proven, the failure modes are disclosed, and synthesis remains gated by evidence. **research-os does not ship a trusted reviewer model by default.** It ships the machinery to prove, reject, or conditionally admit reviewer profiles. Ships: four-stage dogfood swarm (Stages A bug/security, B proactive resilience, C operator humanization, D presentation polish) producing 188 new tests across 23 production-code files, +14 doc/site files, +2 new helper modules (`src/cli/help-topics.ts`, `src/util/progress.ts`); cross-domain handoff doctrine codified after two corrected-forward escapes (A-RE-001 caller migration, C2-RE-001 cross-domain pickup); canonical `--force` sentence anchored byte-for-byte across 8 surfaces with regression test; per-error handbook pointer retrofit on 12 ResearchOSError subclasses; `research-os help <topic>` static recovery surface (4 topics locked); `--no-progress` / `--progress` TTY-detect threading across review/gather/contradict-map/pack-publish with mutex semantics. **No trusted baseline admitted.** **All four frozen packs verify-pack byte-identically against v0.3.3 baselines.** 901/901 vitest passing. Known limitations preserved in [CHANGELOG.md](CHANGELOG.md) and [`handbook/known-limitations.md`](https://mcp-tool-shop-org.github.io/research-os/handbook/known-limitations/): frozen-pack version-stamp historical artifact (B-E-001), npm provenance attestation deferred to v1.x (B-E-004), index schema-version migration is disclosure-based not auto-migration (B-A-003). See [`docs/release-notes/v1.0.0.md`](docs/release-notes/v1.0.0.md) and [`docs/v1-dogfood-swarm-proof.md`](docs/v1-dogfood-swarm-proof.md).
